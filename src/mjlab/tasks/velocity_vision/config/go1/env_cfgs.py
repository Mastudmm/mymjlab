...
from mjlab.tasks.velocity_vision.velocity_env_cfg import make_velocity_env_cfg
...
def unitree_go1_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  # 80x50 resolution, ~86 deg FOV (RealSense D435 approx)
  depth_camera = RayCastSensorCfg(
      name="depth_camera",
      frame=ObjRef(type="site", name="head", entity="robot"), # Ensure robot xml has 'head' site
      pattern=PinholeCameraPatternCfg(width=80, height=80, fovy=86.0),#86改为57
